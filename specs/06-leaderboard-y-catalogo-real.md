...
7. **Guardar score en `app/games/asteroids/play/page.tsx`** — al montar el modal
   ...
   el campo `name`. Al confirmar: - Guardar el nombre en `localStorage.setItem('av_player_name', name)`. - Insertar en `scores` vía `createClient` browser: `{ game_id: 'asteroids',
player_name: name, score, user_id: null }`. - Marcar `saved: true` para deshabilitar el botón y evitar doble envío.
